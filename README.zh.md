...
- **可配置模型** —— 在「设置 → 视觉助手」中选择提供方与模型（或直接编辑 `$DSH_HOME/dsh-vision-helper.json`）；留空自动选择多模态模型。
...
从 `cordis.patch.yml` 移除 `dsh-vision-helper` 行，删除 `<profile>/node_modules/dsh-vision-helper` 文件夹和 `$DSH_HOME/dsh-vision-helper.json`，然后重启。
