window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function apply(ctx) {
			...
			styleEl.setAttribute('data-plugin', 'vision-helper');
			styleEl.textContent = CSS;
			document.head.appendChild(styleEl);
			ctx.effect(() => () => {
				if (styleEl.parentNode !== null) styleEl.parentNode.removeChild(styleEl);
			}, 'vision-helper: styles');

			const ENDPOINT = '/vision-helper/config';
			...
			function VisionSettings() {
				...
				react.useEffect(() => {
					...
					fetch(ENDPOINT).then((r) => r.json()).then((s) => {
						...
						if (!alive) return;
						setError('无法连接宿主配置端点：' + (e && e.message ? e.message : String(e)) + '。请确认 vision-helper 插件已加载。');
					});
					return () => { alive = false; };
				}, []);
				...
				return react.createElement('div', { className: 'vsp-section' },
					...
					react.createElement('p', { className: 'vsp-hint' },
						...
						'配置保存在 $DSH_HOME/vision-helper.json（与 settings.yaml 同级），保存即时生效。提示：若列表里没有合适的视觉模型，可在「模型」设置页添加 OpenAI 兼容提供方后回到本页选择。'
					)
				);
			}
